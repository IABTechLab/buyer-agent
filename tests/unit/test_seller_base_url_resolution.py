...
``SELLER_ENDPOINTS=http://localhost:8001`` but ``POST /products/search``
...
default points at ``http://localhost:3000/api/v2.1`` where nothing listens.
...
class TestResolveSellerBaseUrl:
    def test_prefers_first_seller_endpoint(self, monkeypatch):
        monkeypatch.setenv("SELLER_ENDPOINTS", "http://localhost:8001")
        monkeypatch.delenv("OPENDIRECT_BASE_URL", raising=False)
        assert Settings().resolve_seller_base_url() == "http://localhost:8001"

    def test_first_of_multiple_endpoints_wins(self, monkeypatch):
        monkeypatch.setenv(
            "SELLER_ENDPOINTS",
            "http://seller-a:8001, http://seller-b:8002",
        )
        assert Settings().resolve_seller_base_url() == "http://seller-a:8001"

    def test_falls_back_to_legacy_opendirect_default(self, monkeypatch):
        monkeypatch.delenv("SELLER_ENDPOINTS", raising=False)
        monkeypatch.delenv("OPENDIRECT_BASE_URL", raising=False)
        assert Settings().resolve_seller_base_url() == "http://localhost:3000/api/v2.1"
...
class TestCreateClientUsesResolution:
    ...
    def test_api_create_client_uses_seller_endpoint(self, monkeypatch):
        ...
        monkeypatch.setattr(
            api_main,
            "settings",
            self._fresh_settings(monkeypatch, SELLER_ENDPOINTS="http://localhost:8001"),
        )
    ...
    def test_api_create_client_legacy_fallback(self, monkeypatch):
        ...
        monkeypatch.setattr(
            api_main,
            "settings",
            self._fresh_settings(monkeypatch, OPENDIRECT_BASE_URL="http://localhost:3000/api/v2.1"),
        )
        client = api_main._create_client()
