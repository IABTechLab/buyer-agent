...
def campaign_with_pacing(campaign_store, pacing_store, sample_campaign):
    ...
    pacing_store.save_pacing_snapshot(PacingSnapshot(
        ...
        channel_snapshots=[
            ChannelSnapshot(
                channel="CTV", allocated_budget=30000.0,
                spend=3000.0, pacing_pct=100.0,
                impressions=250000, effective_cpm=12.0, fill_rate=0.85,
            ),
            ChannelSnapshot(
                channel="DISPLAY", allocated_budget=20000.0,
                spend=2000.0, pacing_pct=100.0,
                impressions=200000, effective_cpm=10.0, fill_rate=0.90,
            ),
        ],
        deal_snapshots=[
            DealSnapshot(
                deal_id="deal-001", allocated_budget=15000.0,
                spend=1500.0, impressions=125000,
                effective_cpm=12.0, fill_rate=0.85, win_rate=0.45,
            ),
            DealSnapshot(
                deal_id="deal-002", allocated_budget=15000.0,
                spend=1500.0, impressions=125000,
                effective_cpm=12.0, fill_rate=0.80, win_rate=0.50,
            ),
            DealSnapshot(
                deal_id="deal-003", allocated_budget=10000.0,
                spend=1000.0, impressions=100000,
                effective_cpm=10.0, fill_rate=0.90, win_rate=0.55,
            ),
            DealSnapshot(
                deal_id="deal-004", allocated_budget=10000.0,
                spend=1000.0, impressions=100000,
                effective_cpm=10.0, fill_rate=0.90, win_rate=0.60,
            ),
        ],
    ))
    ...
    pacing_store.save_pacing_snapshot(PacingSnapshot(
        ...
        channel_snapshots=[
            ChannelSnapshot(
                channel="CTV", allocated_budget=30000.0,
                spend=7500.0, pacing_pct=115.0,
                impressions=600000, effective_cpm=12.5, fill_rate=0.82,
            ),
            ChannelSnapshot(
                channel="DISPLAY", allocated_budget=20000.0,
                spend=4500.0, pacing_pct=103.0,
                impressions=430000, effective_cpm=10.47, fill_rate=0.88,
            ),
        ],
        deal_snapshots=[
            DealSnapshot(
                deal_id="deal-001", allocated_budget=15000.0,
                spend=4000.0, impressions=320000,
                effective_cpm=12.5, fill_rate=0.83, win_rate=0.44,
            ),
            DealSnapshot(
                deal_id="deal-002", allocated_budget=15000.0,
                spend=3500.0, impressions=280000,
                effective_cpm=12.5, fill_rate=0.80, win_rate=0.48,
            ),
            DealSnapshot(
                deal_id="deal-003", allocated_budget=10000.0,
                spend=2300.0, impressions=220000,
                effective_cpm=10.45, fill_rate=0.88, win_rate=0.54,
            ),
            DealSnapshot(
                deal_id="deal-004", allocated_budget=10000.0,
                spend=2200.0, impressions=210000,
                effective_cpm=10.48, fill_rate=0.87, win_rate=0.58,
            ),
        ],
    ))
...
def campaign_with_creatives(campaign_store, sample_campaign):
    ...
    campaign_store.save_creative_asset(
        ...
        asset_type="display",
        format_spec=json.dumps({
            "width": 300, "height": 250, "mime_type": "image/png",
        }),
...
class TestCampaignStatusSummary:
    ...
    def test_status_summary_active_campaign(
        self, reporter, sample_campaign, campaign_store
    ):
    ...
    def test_status_summary_draft_campaign(
        self, reporter, sample_draft_campaign
    ):
    ...
    def test_status_summary_with_pacing_data(
        self, reporter, campaign_with_pacing
    ):
    ...
    def test_status_summary_to_json(
        self, reporter, campaign_with_pacing
    ):
    ...
    def test_status_summary_to_text(
        self, reporter, campaign_with_pacing
    ):
...
class TestPacingDashboard:
    ...
    def test_pacing_dashboard_basic(
        self, reporter, campaign_with_pacing
    ):
    ...
    def test_pacing_dashboard_channel_breakdown(
        self, reporter, campaign_with_pacing
    ):
    ...
    def test_pacing_dashboard_deviation_alerts(
        self, reporter, campaign_with_pacing
    ):
        """Pacing dashboard generates alerts when deviation exceeds threshold."""
        dashboard = reporter.pacing_dashboard(
            campaign_with_pacing, deviation_threshold=5.0
        )
    ...
    def test_pacing_dashboard_no_alerts_when_on_pace(
        self, reporter, campaign_with_pacing
    ):
        """Pacing dashboard generates no alerts when threshold is high."""
        dashboard = reporter.pacing_dashboard(
            campaign_with_pacing, deviation_threshold=50.0
        )

        # All deviations are below 50%, so no alerts
        assert len(dashboard.alerts) == 0

    def test_pacing_dashboard_no_snapshots(
        self, reporter, sample_campaign
    ):
    ...
    def test_pacing_dashboard_to_json(
        self, reporter, campaign_with_pacing
    ):
    ...
    def test_pacing_dashboard_to_text(
        self, reporter, campaign_with_pacing
    ):
...
class TestCreativePerformanceReport:
    ...
    def test_creative_report_lists_assets(
        self, reporter, campaign_with_creatives
    ):
        """Creative report lists all assets for a campaign."""
        report = reporter.creative_performance_report(
            campaign_with_creatives
        )
    ...
    def test_creative_report_asset_details(
        self, reporter, campaign_with_creatives
    ):
    ...
    def test_creative_report_pending_validation(
        self, reporter, campaign_with_creatives
    ):
    ...
    def test_creative_report_validation_summary(
        self, reporter, campaign_with_creatives
    ):
    ...
    def test_creative_report_no_assets(
        self, reporter, sample_campaign
    ):
    ...
    def test_creative_report_to_json(
        self, reporter, campaign_with_creatives
    ):
    ...
    def test_creative_report_to_text(
        self, reporter, campaign_with_creatives
    ):
...
class TestDealReport:
    ...
    def test_deal_report_aggregate_stats(
        self, reporter, campaign_with_pacing
    ):
    ...
    def test_deal_report_no_snapshots(
        self, reporter, sample_campaign
    ):
...
class TestCampaignReport:
    ...
    def test_full_report_for_draft_campaign(
        self, reporter, sample_draft_campaign
    ):
...
class TestEdgeCases:
    ...
    def test_pacing_deviation_critical_alert(
        self, reporter, campaign_store, pacing_store
    ):
        ...
        pacing_store.save_pacing_snapshot(PacingSnapshot(
            ...
            channel_snapshots=[
                ChannelSnapshot(
                    channel="CTV", allocated_budget=100000.0,
                    spend=60000.0, pacing_pct=200.0,
                    impressions=5000000, effective_cpm=12.0, fill_rate=0.95,
                ),
            ],
        ))
