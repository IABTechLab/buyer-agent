...
class TestMetaCampaignsEndpoint:
    def test_unconfigured_returns_503(self, allow_operator_auth):
        with _meta_unconfigured():
...
class TestMetaReportEndpoint:
    ...
    def test_empty_campaign_ids_returns_400(self, allow_operator_auth):
        with _meta_configured():
            response = _client().get("/meta/report", params={"campaign_ids": " , "})
        assert response.status_code == 400

    def test_error_response_never_leaks_token(self, allow_operator_auth):
        with _meta_configured():
...
class TestCampaignReportEndpoint:
    def test_unknown_job_returns_404(self, allow_operator_auth):
        response = _client().get("/reports/nonexistent-job")
        assert response.status_code == 404

    def test_job_without_booked_lines(self, allow_operator_auth):
    ...
    def test_seller_lines_keyed_by_deal_id(self, allow_operator_auth):
    ...
    def test_meta_lines_unconfigured_message(self, allow_operator_auth):
