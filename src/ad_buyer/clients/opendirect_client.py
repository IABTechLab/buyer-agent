...
class OpenDirectClient:
    ...
    async def list_products(self, skip: int = 0, top: int = 50, **filters: Any) -> list[Product]:
        ...
        async with self._make_client() as http:
            response = await http.get("/products", params=params)
    ...
    async def search_products(self, filters: dict[str, Any]) -> list[Product]:
        ...
        async with self._make_client() as http:
            response = await http.post("/products/search", json=filters)
            response.raise_for_status()
        data = response.json()
        products = data.get("products", data) if isinstance(data, dict) else data
        return [Product.model_validate(p) for p in products]
    ...
    async def get_account(self, account_id: str) -> Account:
        ...
        """
        async with self._make_client() as http:
            response = await http.get(f"/accounts/{account_id}")
            response.raise_for_status()
        return Account.model_validate(response.json())
    ...
    async def list_accounts(self, skip: int = 0, top: int = 50) -> list[Account]:
        ...
        async with self._make_client() as http:
            response = await http.get("/accounts", params=params)
            response.raise_for_status()
        data = response.json()
    ...
    async def get_order(self, account_id: str, order_id: str) -> Order:
        ...
        """
        async with self._make_client() as http:
            response = await http.get(f"/accounts/{account_id}/orders/{order_id}")
            response.raise_for_status()
        return Order.model_validate(response.json())
    ...
    async def list_orders(self, account_id: str, skip: int = 0, top: int = 50) -> list[Order]:
        ...
        params = {"$skip": skip, "$top": top}
        async with self._make_client() as http:
            response = await http.get(f"/accounts/{account_id}/orders", params=params)
            response.raise_for_status()
        data = response.json()
    ...
    async def get_creative(self, account_id: str, creative_id: str) -> Creative:
        ...
        """
        async with self._make_client() as http:
            response = await http.get(f"/accounts/{account_id}/creatives/{creative_id}")
            response.raise_for_status()
        return Creative.model_validate(response.json())
    ...
    async def list_creatives(
        ...
        params = {"$skip": skip, "$top": top}
        async with self._make_client() as http:
            response = await http.get(f"/accounts/{account_id}/creatives", params=params)
            response.raise_for_status()
        data = response.json()
    ...
    async def close(self) -> None:
        """No-op — each request manages its own client lifetime."""
