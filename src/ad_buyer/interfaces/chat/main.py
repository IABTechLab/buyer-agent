...
class MultiSellerSearchTool(BaseTool):
    ...
    description: str = (
        "Search for advertising inventory across ALL connected seller agents. "
        "Use this to find products, check availability, and compare offerings "
        "from multiple publishers/SSPs. Returns aggregated results from all sellers."
    )
            ...
            if "error" in r:
                ...
                if isinstance(products, list):
                    for p in products[:5]:  # Limit to 5 per seller
                        ...
                        output.append(
                            f"  - {name} | {publisher} | {channel} | ${price} CPM | {avail_str} avail"  # noqa: E501
                        )  # noqa: E501
                else:
