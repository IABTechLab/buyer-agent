...
# Only include beads with this prefix in the generated PROGRESS.md.
# Each repo sets its own prefix: buyer- for ad_buyer_system, seller- for
# ad_seller_system, ar- for the parent agent_range repo.  Beads from other
# repos arrive via `bd sync` but should not appear in this repo's progress.
BEAD_PREFIX = "buyer-"
...
def generate():
    ...
    # Filter to only this repo's beads (by prefix)
    if BEAD_PREFIX:
        issues = [i for i in issues if i.get("id", "").startswith(BEAD_PREFIX)]
